...
function BTR_Fire_Inspection_Scheduled() {
    try {
        ...
        if (wfTask == "Fire Review" && wfStatus == "Inspection Required") {
            scheduleInspection("Fire Inspection", 1);
...
function BTR_Assess_BTR_Fee(){
  ...
  var TotalFireFee = AInfo['Total Fire Fees'];
    if(wfTask == "Building Review" && matches(wfStatus, 'Approved', 'Completed Change of Use Required', 'Completed No Issue')) {
      var today = new Date();
      var aprilFirst = "04/01/" + today.getFullYear();
      var octFirst = "10/01/"+ today.getFullYear();
      var date1 = new Date(aprilFirst); logDebug("test1: " + dateDiff(today, date1));
      var date2 = new Date(octFirst);   logDebug("test2: " + dateDiff(today, date2));
      if(dateDiff(today, date1) < 0 && dateDiff(today, date2) > 0){
        updateFee(BTR, feeSchedule, 'FINAL', (parseFloat(BTRTotalLicFee)/2), "N");
        editAppSpecific("Half Year License Fee", "CHECKED");
        updateFee(Fire, feeSchedule, 'FINAL', TotalFireFee, "N");
      }else{
        updateFee(BTR, feeSchedule, 'FINAL', BTRTotalLicFee, "N");
        updateFee(Fire, feeSchedule, 'FINAL', TotalFireFee, "N");   
      }
    ...
    try {
        ...
        if (appMatch("Licenses/Business/Tax Receipt/Application")) {
            ...
            if (prePay) {
                ...
                expDate.getFullYear();  //set expiration year to current year
            }
...
function BD_Submittal_Fee() {
    try {
        ...
        if (feeExists('COMMASUB')) updateFee('COMMASUB', 'BD_PERMITS', 'FINAL', 0, "Y");
        if (use) {
            if (use == "Commercial") {
...
function PL_SitePlan_Application_Fees() {
    try {
        ...
        loadAppSpecific(AInfo, capId);
        var applType = AInfo["Type of Site Plan"];
        var acres = 1;
        var acresValue = AInfo["Acres"];
...
function assessImpactFees() {
    try {
        ...
        var transientAssistedHomeCat = AInfo["Transient, Assisted Home Category"];
        ...
        if (wfTask == "Review Consolidation" && wfStatus == "Completed") {
            if (calcImpactFees == "Yes") {
                ...
                if (policeImpactCat) {
                    var policeLookUpFee = lookup("Police_Impact_Category", policeImpactCat);
                    if (policeLookUpFee) {
                ...
                if (parkRecImpactFee) {
                    ...
                    if (parkRecImpactFee == "Single Family") {
                ...
                if (officeImpact == "Medical Office") {
                    updateFee("OFFICEMED", "IMPACT", "FINAL", officeSquareFeet / 1000, "N");
                }
                //assess Retail impact cat fee
                if (retailImpactCat) {
                    var retailImpactCatFee = lookup("Retail_Impact_Category", retailImpactCat);
                    var totalFee = parseFloat(retailSquareFeet / 1000);
                    if (retailImpactCatFee) {
                ...
                if (instituionImpactCat) {
                    ...
                    var institutionImpactCatFee = lookup("Institution_Impact_Category", instituionImpactCat);
                ...
                if (industrialImpactCat) {
                    ...
                    var industrialImpactCatFee = lookup("Industrial_Impact_Category", industrialImpactCat);
                ...
                if (recreationalImpactCat) {
                    ...
                    var recreationalImpactCatFee = lookup("Recreational_Impact_Category", recreationalImpactCat);
                ...
                if (transientAssistedHomeCat) {
                    ...
                    var transientImpactCatFee = lookup("Transient_Impact_Category", transientAssistedHomeCat);
        ...
        showMessage = true;
        comment("ERROR: ASA function assessImpactFees(), please contact administrator. Error: " + err);
    }
}
...
function scheduleInspectionsIssuedStatusISB() {
    try {
        if (!matches(capStatus, "Issued")) {
            cancel = true;
            showMessage = true;
            comment("No inspections can be scheduled until permit has been issued. Please contact Permit Clekrs at 386-626-7006 to get more information");
        }
        if (balanceDue > 0) {
            cancel = true;
            showMessage = true;
...
function addLayersAttributes() {
    try {
        var zon2 = getGISInfo("Accela/Accela_Basemap", "Zoning District Boundaries", "GENL_ZCODE");
		var zoned = getGISInfo("Accela/Accela_Basemap", "Zoning District Boundaries", "Zoning_Dis");
        //var futureLandUse = getGISInfo("Accela/Accela_Basemap", "Future Land Use", "Future Land Use Code");
        var futureLandUse = getGISInfo("Accela/Accela_Basemap", "Future Land Use", "FLU_NAME");
		var overlay = getGISInfo("Accela/Accela_Basemap", "Future Land Use Overlays", "Overlay");

        editAppSpecific("Zoned", zoned);
        editAppSpecific("Future Land Use", futureLandUse);
		editAppSpecific("Primary Land Use", futureLandUse);
        editAppSpecific("District Overlay", overlay);
		editAppSpecific("Institutional", zon2);
    }
    catch (err) {
        showMessage = true;
...
function PreventSubmission_taxDistrictNum() {
    try {
        ...
        var taxDist = getGISInfo("Accela/Accela_Basemap", "Parcels", "TAXDIST"); logDebug("taxDist: " + taxDist);
...
function PL_Variance_Application_Fees() {
    try {
        ...
        if ($iTrc(existingUse == "Single-family dwellings", 'existingUse == "Single-family dwellings"')) {
            ...
            updateFee(vFeeItemothers, vFeeSched, "FINAL", 1, "N");
        }
